...
migrate((db) => {
  ...
  collection.updateRule = "(user = @request.auth.id || (@collection.location.id ?= location && @collection.location.users ?~ @request.auth.id)) && @request.data.end > @request.data.start"
